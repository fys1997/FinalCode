...
class GcnAttentionCell(nn.Module):
    ...
    def forward(self, hidden, tXin, matrix):
        ...
        # 做attention
        atten_mask = GcnAttentionCell.generate_square_subsequent_mask(B=query.size(0), N=query.size(1),
                                                                      T=query.size(2)).to(
            self.device)  # batch*N*1*Tq*Ts
        value, atten = self.temporalAttention.forward(query=query, key=key, value=value, atten_mask=atten_mask,
                                                      tX=tXin[:, 0, :, :])  # batch*N*T*dmodel
...
class GcnEncoder(nn.Module):
    ...
    def forward(self, x, tx):
        ...
        skip = 0
        matrix = self.GCNMeta(t=tx[:, 0, :, :])  # N*N
...
class GcnDecoder(nn.Module):
    ...
    def forward(self, x, ty):
        ...
        skip = 0
        matrix = self.GCNMeta(t=ty[:, 0, :, :])  # N*N
...
class TemMulHeadAtte(nn.Module):
    ...
    def forward(self, query, key, value, atten_mask, tX):
        ...
        if self.normalTA:
            query = F.relu(self.dropout(self.query_projection(query)).view(B, N, T, H, -1)) # batch*N*T*heads*d_keys
            key = F.relu(self.dropout(self.key_projection(key)).view(B, N, T, H, -1)) # batch*N*T*heads*d_keys
            value = F.relu(self.dropout(self.value_projection(value)).view(B, N, T, H, -1)) # batch*N*T*heads*d_keys
        else:
        ...
        if self.normalTA:
            value = torch.sigmoid(self.out_projection(value))
        else:
